...
def get_old_attendance_data():
    driver = "ODBC+Driver+18+for+SQL+Server&TrustServerCertificate=yes"
    engine = db.create_engine(f"mssql+pyodbc://{username}:{password}@{server}/{database}?driver={driver}")
    conn = engine.connect()

    sql_query = "SELECT * FROM SJSD_CSS.dbo.AttendanceArchive WHERE SchoolYear = 2024"
    ...
    for student in list_of_students:
        ...
        for term in terms:
            ...
            student_attendance_data[f'23-24 Term {term} Tardies'] = total_tardy
            student_attendance_data[f'23-24 Term {term} Total Absences'] = total_absences / total_periods
            student_attendance_data[f'23-24 Term {term} Total Excused Absences'] = total_excused / total_periods
            student_attendance_data[f'23-24 Term {term} Total Unexcused Absences'] = total_unexcused / total_periods
