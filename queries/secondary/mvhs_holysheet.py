...
def update_mvhs_data():

    query = {
        "School Name" : "Monument Valley High School"
    }
    ...
    # Open Google Sheet
    mvhs_worksheet = gc.open('MVHS Holysheet')
    mvhs_sheet = mvhs_worksheet.worksheet('data')

    return mvhs_sheet.update([df.columns.values.tolist()] + df.fillna('').values.tolist())